class dropboxController {
    constructor() {
        ...
        this.fileListEl = document.getElementById('list-of-files-and-directories')
    ...
    getSelection() {
        return this.fileListEl.querySelectorAll('.selected')
    }
    ...
    initEvents() {
        ...
        this.btnRename.addEventListener('click', e => {
            ...
            let newName = prompt('Renomear o arquivo para: ', file.name)
        ...
        this.fileListEl.addEventListener('selectionchange', e => {
        ...
        this.inputFilesEl.addEventListener('change', e => {
            ...
            this.updateTask(e.target.files).then(responses => {
    ...
    uploadProgress(e, file) { 
        ...
        this.progressBarEl.style.width = `${uploadPercentage}%`

        let timeSpent = Date.now() - this.startUploadTime
        let timeLeft = ((100 - uploadPercentage) * timeSpent) / uploadPercentage

        let result = this.formatTimeForHuman(timeLeft)

        this.fileNameEl.textContent = file.name
        this.timeLeftEl.textContent = result
    }
    ...
    readFiles() {
        ...
        this.getFirebaseRef().on('value', snapshot => {
            ...
            snapshot.forEach(item => {
                ...
                if(data.type) {
                    this.fileListEl.appendChild(this.getFileView(data, key))
                }
            })
        })
    }
    ...
    initEventsLi(li) {
        ...
        li.addEventListener('click', e => {
            if(e.shiftKey) {
                let firstLi = this.fileListEl.querySelector('.selected')
                ...
                this.fileListEl.dispatchEvent(this.onSelectionChange)

                return
            }

            if(!e.ctrlKey) {
                this.fileListEl.querySelectorAll('li.selected')
                .forEach(item => {
