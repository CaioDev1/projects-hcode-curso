class dropbox {
    constructor() {
        ...
        this.uploadBarEl = document.getElementById('react-snackbar-root')

        this.uploadBarTitle = this.uploadBarEl.querySelector('.filename')
        this.uploadBarTime = this.uploadBarEl.querySelector('.timeleft')
        this.uploadBarLoading = this.uploadBarEl.querySelector('.mc-progress-bar-fg')

        this.fileList = document.getElementById('list-of-files-and-directories')
    ...
    firebaseInit() {
        var firebaseConfig = {
            apiKey: "AIzaSyCSwUVqk3k3H5zbjKbRieDnZAuiXzR1in4",
            authDomain: "dropboxprojecthcode.firebaseapp.com",
            databaseURL: "https://dropboxprojecthcode.firebaseio.com",
            projectId: "dropboxprojecthcode",
            storageBucket: "dropboxprojecthcode.appspot.com",
            messagingSenderId: "716116791330",
            appId: "1:716116791330:web:a28c50787e6b9b81d43a7b",
            measurementId: "G-C4G3JFZRSM"
          };
    ...
    getItems() {
        return this.fileList.querySelectorAll('.selected')
    }
    ...
    initEvents() {
        ...
        this.btnDelete.addEventListener('click', e => {
            this.removeTask().then(response => {
                response.forEach(item => {
                    if(item.fields.key) {
                        this.firebaseRef().child(item.fields.key).remove()
                    }
                })
            }).catch(err => console.log(err))
        })

        this.fileList.addEventListener('selectionchange', e => {
            switch(this.getItems().length) {
                case 0:
        ...
        this.btnRename.addEventListener('click', e => {
            let li = this.fileList.querySelector('.selected')

            let file = JSON.parse(li.dataset.file)

            let newName = prompt('Novo nome: ', file.name)
        ...
        this.inputFilesEl.addEventListener('change', e => {
            ...
            this.uploadToServer(e.target.files).then(response => {
                response.map(file => {
    ...
    showUploadBar(option=true) {
        this.uploadBarEl.style.display = option ? 'block' : 'none'
    }
    ...
    updateProgress(e, file) {
        ...
        this.uploadBarLoading.style.width = `${uploadPercentage}%`

        let timespent = Date.now() - this.startUploadTime
        let timeleft = ((100 - uploadPercentage) * timespent) / uploadPercentage

        this.uploadBarTitle.textContent = file.name
        this.uploadBarTime.textContent= this.formatTime(timeleft)
    }
    ...
    initEventsLi(li) {
        ...
        li.addEventListener('click', e => {
            if(e.shiftKey) {
                let firstLi = this.fileList.querySelector('.selected')

                if(firstLi) {
                    let firstIndex, lastIndex

                    this.fileList.childNodes.forEach((el, index) => {
                        if(el == firstLi) firstIndex = index
                        if(el == li) lastIndex = index
                    })

                    let index = [firstIndex, lastIndex].sort()

                    this.fileList.childNodes.forEach((item, i) => {
                ...
                this.fileList.dispatchEvent(this.selectionChange)
    ...
    navRender() {
        ...
        for(let i = 0; i < this.currentFolder.length; i++) {
            ...
            if(i + 1 == this.currentFolder.length) {
                ...
                <svg width="24" height="24" viewBox="0 0 24 24" class="mc-icon-template-stateless" style="top: 4px; position: relative;">
                    ...
                    <path d="M10.414 7.05l4.95 4.95-4.95 4.95L9 15.534 12.536 12 9 8.464z" fill="#637282" fill-rule="evenodd" data-darkreader-inline-fill="" style="--darkreader-inline-fill:#a0978b;"></path>
                </svg>
                `
            }
        ...
        this.navEl.querySelectorAll('a').forEach(el => {
            el.addEventListener('click', e => {
                ...
                this.currentFolder = el.dataset.path.split('/')
    ...
    readFiles() {
        ...
        this.firebaseRef().on('value', data => {
            ...
            data.forEach(item => {
                ...
                if(file.type) {
                    this.fileList.appendChild(this.createLi(key, file))
                }
            })
        })
    }
